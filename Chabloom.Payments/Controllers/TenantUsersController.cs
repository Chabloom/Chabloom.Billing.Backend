...
namespace Chabloom.Payments.Controllers
{
    ...
    public class TenantUsersController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<TenantUserViewModel>>> GetTenantUsers(Guid? tenantId)
        {
            ...
            List<TenantUserViewModel> tenantUsers;
            if (tenantId == null)
            {
                // Find all tenantUsers the user has access to
                tenantUsers = await _context.TenantUsers
                    .Include(x => x.Role)
                    .Include(x => x.Tenant)
                    .ThenInclude(x => x.Users)
                    .Where(x => x.Tenant.Users.Select(y => y.UserId).Contains(userId))
                    .Where(x => !x.Disabled)
        ...
        public async Task<ActionResult<TenantUserViewModel>> GetTenantUser(Guid id)
        {
            ...
            // Find the specified tenant user if the user has access to it
        ...
        public async Task<IActionResult> PutTenantUser(Guid id, TenantUserViewModel viewModel)
        {
            ...
            {
                _logger.LogWarning($"User id {userId} did not have permissions to update tenant user for tenant {tenantUser.Tenant.Id}");
                return Forbid();
            }
